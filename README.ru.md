...
* 📶 **Диагностика DSL и Mesh**: Разбор сырой статистики VDSL2/ADSL драйвера в читаемые таблицы SNR/затухания/ошибок (`dsl stats`, `dsl reset`), список узлов-повторителей Mesh Wi-Fi System (MWS) с CPU/RAM и данными backhaul (`mesh`).
* 🔄 **Режим live-мониторинга**: Добавьте `--watch` к `hosts`, `mesh` или `dsl stats` для автообновляемого экрана в терминале.
...
# Показать узлы-повторители Mesh Wi-Fi System (или перезагрузить узел по CID)
keenetic --password "ваш_пароль" mesh
keenetic --password "ваш_пароль" mesh reboot <cid>

# Диагностика линии DSL (SNR, затухание, счетчики ошибок) или сброс линии
keenetic --password "ваш_пароль" dsl stats
keenetic --password "ваш_пароль" dsl reset

# Автообновляемый экран (работает с hosts / mesh / dsl stats)
keenetic --password "ваш_пароль" hosts --watch --interval 5
